...
function Home() {
  const { data, loading, error, refetch } = useFetch('https://api.alquran.cloud/v1/ayah/random/en.sahih');
        ...
        <div className="bg-gradient-to-br from-emerald-900/80 via-emerald-800/60 to-emerald-700/40 backdrop-blur-sm border border-emerald-600/30 rounded-2xl p-8 shadow-2xl">
          ...
          {loading ? (
            ...
            <div className="text-red-300 mb-4">
              <p>{error.message || "Unable to load verse. Please try again."}</p>
              <button 
                onClick={refetch}
                className="mt-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors cursor-pointer"
              >
                Try Again
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-emerald-200">
                ...
                <button 
                  onClick={refetch}
                  className="px-4 py-2 bg-emerald-600/80 hover:bg-emerald-600 text-white rounded-lg transition-colors text-sm font-medium cursor-pointer"
                >
                  New Verse
                </button>
              </div>
